...
def protect_client():
    ...
    # Generate protected code using PyArmor (version-compatible)
    cmd = [
        "pyarmor", "gen",
        "--output", str(dist_dir),
        "--recursive",
    ...
    if result.returncode == 0:
        ...
        for file in dist_dir.rglob("*.py"):
            print(f"  - {file}")
        print("\n⚠️ Make sure to copy the generated `pyarmor_runtime_000000` folder into dist/backend if it's outside.")
    else:
